...
# ── Step 2: 同步 charts.html + embedded JS → embedded/ 副本 ─────────────
if MODE in ("full", "charts"):
    print("\n\n>>> Step 2: 同步 charts.html + embedded JS → embedded/")
    ...
    if not os.path.exists(embedded_dir):
        ...
        # 2b. 同步 embedded JS（解决 database_hub.html 读取 embedded/ 下文件但 import_all.py 只写根目录的问题）
        for js, _ in DB_FILES:
            src = os.path.join(BASE, js)
            dst = os.path.join(embedded_dir, js)
            if os.path.exists(src):
                with open(src, 'r', encoding='utf-8') as f:
                    content = f.read()
                with open(dst, 'w', encoding='utf-8') as f:
                    f.write(content)
                # 从 embedded JS 中提取 update_time 便于日志
                import re as re2
                m = re2.search(r'"update_time"\s*:\s*"([^"]+)"', content)
                ts = m.group(1) if m else 'unknown'
                print(f"  [sync] {js} → embedded/  (update_time: {ts})")
            else:
                print(f"  [skip] {js} (源文件不存在，import_all.py 尚未生成)")
else:
...
if MODE in ("full", "charts"):
    ...
    to_add += ["embedded/" + c for _, c in DB_FILES]             # 7个 embedded/charts.html
    to_add += [p.replace(BASE + "/", "").replace("\\","/") for p in PAGES]  # 5个引用页
    to_add += [js for js, _ in DB_FILES]                          # 7个根目录 embedded JS
    to_add += ["embedded/" + js for js, _ in DB_FILES]            # 7个 embedded/*.js（解决 database_hub 读取问题）
