...
def git_commit_data(today):
    """立即将更新后的数据文件提交到 git，消除“未提交窗口”被后续 git 恢复操作抹除的风险。

    根因防护（2026-07-11 新增）：本脚本仅写工作目录、不提交，若之后有 git restore/checkout
    等恢复操作，未提交的当日数据会被还原为上一次提交版本而丢失（2026-07-10 即因此丢失 07-10 数据）。
    提交后即使发生普通 restore 也不会回退已固化的数据。
    """
    import subprocess
    repo = Path(__file__).parent.absolute()
    files = ["electrolyte_data.js", "index_v3.html", "index.html"]
    try:
        subprocess.run(["git", "add"] + files, cwd=repo, check=False,
                       capture_output=True, text=True)
        r = subprocess.run(
            ["git", "commit", "-m", f"auto: 电解液数据更新 {today}"],
            cwd=repo, capture_output=True, text=True
        )
        out = (r.stdout or "") + (r.stderr or "")
        if r.returncode == 0:
            print(f"  已 git 提交电解液数据（{today}），数据已固化，不会被后续恢复操作抹除")
        elif "nothing to commit" in out:
            print("  git 无变更需提交（数据已是最新提交状态）")
        else:
            print(f"  警告: git 提交未成功（不影响本地数据）: {out.strip()}")
    except Exception as e:
        print(f"  警告: git 提交异常（不影响本地数据）: {e}")
...
def run_update():
    ...
    # 7. 立即 git 提交固化数据，防止未提交窗口被后续 git 恢复操作抹除
    print("\n固化数据到 git...")
    git_commit_data(today)
