...
def validate_static_html(html_file):
    ...
    # 2. 检查数据加载方式
    # 方案A: window.__EMBEDDED__（嵌入式）
    # 方案B: fetch 动态加载（在 HTML 或关联的 inline_*.js 中）
    embedded_assignments = re.findall(r'window\.__EMBEDDED__\s*=\s*\{', content)
    embedded_count = len(embedded_assignments)
    
    # 检查 HTML 中是否有 fetch
    has_fetch = 'fetch(' in content and 'reports/' in content
    
    # 检查关联的 inline JS 文件是否有 fetch
    if not has_fetch:
        inline_js_files = list(PROJECT_ROOT.glob("inline_*.js"))
        for js_file in inline_js_files:
            try:
                with open(js_file, 'r', encoding='utf-8') as f:
                    js_content = f.read()
                    if 'fetch(' in js_content and 'reports/' in js_content:
                        has_fetch = True
                        break
            except:
                pass
    
    if embedded_count == 0 and not has_fetch:
        raise ValidationError("未找到数据加载方式（window.__EMBEDDED__ 或 fetch）")
    
    if embedded_count > 1:
        raise ValidationError(f"找到多个 window.__EMBEDDED__ ({embedded_count} 个)")
    
    if embedded_count == 1:
        log(f"  数据加载方式: 嵌入式 (window.__EMBEDDED__)", "SUCCESS")
    elif has_fetch:
        log(f"  数据加载方式: fetch 动态加载", "SUCCESS")
    ...
    try:
        ...
        if reports_dir.exists():
            report_files = sorted(reports_dir.glob("2026-*.json"))
            # 过滤掉子部门报告
            report_files = [f for f in report_files if '-lpsd' not in f.name and '-lubricant' not in f.name and '-sdmd' not in f.name and '-sjl' not in f.name]
            ...
            if report_files:
                ...
                today = data.get('date', '')
                if not today:
                    raise ValidationError("未找到数据日期 (date 字段)")
                ...
                # 检查事业部ID
                expected_ids = ['bych', 'czly', 'dhx', 'felt', 'fnlt', 'kelan', 'lpsd', 'lubricant', 'sdmd', 'sjl']
                actual_ids = list(departments.keys())
                wrong_ids = [id for id in actual_ids if id not in expected_ids]
                
                if wrong_ids:
                    log(f"  警告: 未知事业部ID: {wrong_ids}", "WARNING")
                
                log(f"  最新报告: {latest_report.name}", "SUCCESS")
                
                return data
            else:
                log(f"  警告: 未找到报告文件，跳过数据校验", "WARNING")
        else:
            log(f"  警告: reports 目录不存在，跳过数据校验", "WARNING")
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            has_data = page.evaluate("""() => {
                return window.__EMBEDDED__ !== undefined || document.querySelector('.stat-card') !== null;
            }""")
            
            if not has_data:
                log(f"  警告: 数据可能未渲染", "WARNING")
            else:
