...
def optimize_html(html_path, output_dir=None):
    ...
    # ══════════════════════════════════════════════════════════════
    # 重要：inline_00.js 和 inline_01.js 是手动维护的核心运行时文件，
    #       包含 HTML_VERSION、fetch 去重、Service Worker 注册等关键代码，
    #       禁止被覆盖。从 inline_02.js 开始生成提取的脚本块。
    # ══════════════════════════════════════════════════════════════
    PROTECTED_FILES = {'inline_00.js', 'inline_01.js'}

    for idx, (start, end, script_body) in enumerate(sorted_blocks):
        # 跳过受保护的文件（从 inline_02 开始）
        js_fname = f"inline_{len(sorted_blocks)-1-idx:02d}.js"
        if js_fname in PROTECTED_FILES:
            print(f"    [保护] 跳过 {js_fname}（核心运行时文件，禁止覆盖）")
            continue
