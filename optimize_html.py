#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
对指定的 HTML 文件做「轻量化处理」：
1. 提取所有内联 <script> 块为外部 inline_XX.js
2. 替换 <script> 标签为 <script src="inline_XX.js"></script>
3. 提取 data:image/...;base64, 为外部图片文件

用法: python optimize_html.py <html_file_path>
"""

import re
import os
import sys
import base64

def optimize_html(html_path, output_dir=None):
    """
    对 HTML 文件做轻量化处理，覆盖原文件。
    提取的内联脚本放在 html_path 同目录（或 output_dir）。
    """
    if output_dir is None:
        output_dir = os.path.dirname(html_path)

    with open(html_path, "r", encoding="utf-8") as f:
        content = f.read()

    original_size = len(content.encode("utf-8"))
    print(f"处理文件: {html_path}")
    print(f"原始大小: {original_size:,} bytes ({original_size/1024:.1f} KB)")

    # ── 第1步：提取 data:image base64 为外部文件 ──────────
    img_dir = os.path.join(output_dir, "img")
    os.makedirs(img_dir, exist_ok=True)

    def replace_base64_img(m):
        full_match = m.group(0)
        mime = m.group(1)
        b64data = m.group(2)
        ext = mime.split("/")[1]
        if "png" in ext: ext = "png"
        elif "jpeg" in ext or "jpg" in ext: ext = "jpg"
        elif "gif" in ext: ext = "gif"
        elif "webp" in ext: ext = "webp"
        elif "svg" in ext: ext = "svg"
        else: ext = "png"

        # 去重：相同 base64 内容只存一次
        content_hash = str(abs(hash(b64data)))
        fname = f"inline_img_{content_hash[:8]}.{ext}"
        fpath = os.path.join(img_dir, fname)
        if not os.path.exists(fpath):
            with open(fpath, "wb") as out:
                out.write(base64.b64decode(b64data))
        # 替换为本页相对路径
        relpath = os.path.relpath(fpath, os.path.dirname(html_path))
        relpath = relpath.replace("\\", "/")
        return f"img/{os.path.basename(fpath)}"

    img_pattern = re.compile(r'data:(image/[^;]+);base64,([A-Za-z0-9+/=]+)')
    content, n_img = img_pattern.subn(replace_base64_img, content)
    if n_img:
        print(f"  ✅ 提取 {n_img} 处 base64 图片 → {img_dir}")

    # ── 第2步：提取内联 <script> 块 ──────────────────────
    # 找到所有 <script> 标签位置（包括有 src= 的）
    script_pattern = re.compile(r'<script\b[^>]*>(.*?)</script>', re.DOTALL)

    # 收集所有需要提取的内联块（无 src= 属性的）
    inline_blocks = []
    for m in script_pattern.finditer(content):
        tag_start = m.start()
        tag_text = m.group(0)
        script_body = m.group(1)
        # 检查这个 <script> 标签是否有 src= 属性
        tag_open = re.match(r'<script\b([^>]*)>', tag_text)
        has_src = tag_open and 'src=' in tag_open.group(1)
        if not has_src and len(script_body.strip()) > 0:
            inline_blocks.append((m.start(), m.end(), script_body))

    if not inline_blocks:
        print("  ℹ️  无内联脚本需要提取")
        return original_size

    print(f"  📦 发现 {len(inline_blocks)} 个内联脚本块，开始提取...")

    # 从后往前替换，保持位置正确
    new_content = content
    offset = 0
    extracted_files = []

    for idx, (start, end, script_body) in enumerate(reversed(inline_blocks)):
        adj_start = start + offset
        adj_end = end + offset
        js_fname = f"inline_{idx:02d}.js"
        js_path = os.path.join(output_dir, js_fname)

        # 写 JS 文件（追加模式，因为 reversed 顺序）
        with open(js_path, "w", encoding="utf-8") as f:
            f.write(script_body)

        fsize = os.path.getsize(js_path)
        extracted_files.append((js_fname, fsize))
        print(f"    [{idx:02d}] {fsize:,} bytes → {js_fname}")

        # 替换原内联块为 src 引用
        relpath = js_fname  # 同目录，直接文件名
        replacement = f'<script src="{relpath}"></script>'
        new_content = new_content[:adj_start] + replacement + new_content[adj_end:]
        offset += len(replacement) - (adj_end - adj_start)

    # 写回 HTML
    with open(html_path, "w", encoding="utf-8") as f:
        f.write(new_content)

    new_size = os.path.getsize(html_path)
    print(f"\n✅ 优化完成:")
    print(f"  HTML: {original_size:,} → {new_size:,} bytes ({new_size/1024:.1f} KB)")
    print(f"  减少: {(original_size-new_size):,} bytes ({(original_size-new_size)/original_size*100:.1f}%)")
    print(f"  提取 JS 文件: {len(extracted_files)} 个")
    total_js = sum(s[1] for s in extracted_files)
    print(f"  JS 文件合计: {total_js:,} bytes ({total_js/1024:.1f} KB)")

    return original_size


if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python optimize_html.py <html_file_path> [output_dir]")
        sys.exit(1)
    html_file = sys.argv[1]
    out_dir = sys.argv[2] if len(sys.argv) > 2 else None
    optimize_html(html_file, out_dir)
