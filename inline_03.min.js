...
logger.info(`[loadReport] 开始加载 ${date}.json...`);
const resp = await fetchWithTimeout(`reports/${date}.json?v=${HTML_VERSION}&_cb=${Date.now()}`, 5000);
logger.info(`[loadReport] HTTP状态: ${resp.status}, url=${resp.url}`);
if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
const data = await resp.json();
logger.info(`[loadReport] 成功加载 ${date}，${Object.keys(data.departments || {}).length} 个事业部`);
if (date === TODAY) {
...
if (!sections) return '';
logger.info('[sectionsToHTML] ENTRY: sections.length=' + (sections.length || 0) + ', windowEnd=' + windowEnd);
...
logger.info('[renderBody] title=' + (sec.title || sec.name) + ', tableLines.length=' + tableLines.length + ', tableHtml.length=' + tableHtml.length);
html += tableHtml;
}
const bodyText = bodyLines.join('\n').trim();
logger.info('[renderBody] title=' + (sec.title || sec.name) + ', bodyText前200=', String(bodyText.substring(0, 200)));
if (bodyText) {
if (title === '今日关注') {
...
logger.info('[renderBody] title=' + (sec.title || sec.name) + ', colonTitleMatches.length=' + colonTitleMatches.length);
...
logger.info('[sectionsToHTML] EXIT: html.length=' + html.length + ', sections processed=' + sections.length);
return html;
}
const dimLabelMap = {
topnews:    ['今日要报', '📌'],
...
logger.info('[buildDynamicReportData] sectionsToHTML returned, html.length=' + html.length + ', sections.length=' + deptData.sections.length + ', deptId=' + deptId);
...
logger.info('[buildDynamicReportData] sectionsToHTML (normalized object format) returned, html.length=' + html.length + ', deptId=' + deptId);
if (deptData.risk_tip || deptData.risk) {
html += `<div class="report-risk"><b>⚠️ 风险提示：</b>${deptData.risk_tip || deptData.risk}</div>\n`;
}
result[deptId] = {
...
logger.info('[initDynamicData] embedded 模式，使用内嵌数据');
try {
dynamicReportData = buildDynamicReportData(window.__EMBEDDED__.report);
logger.info('[initDynamicData] buildDynamicReportData 成功，keys:', String(Object.keys(dynamicReportData || {})));
} catch(e) {
logger.error('[initDynamicData] buildDynamicReportData 失败:', String(e));
...
logger.info(`[initDynamicData] 网络模式，尝试加载 ${todayStr}.json...`);
try {
const jsonData = await loadReportJSON(todayStr);
logger.info('[initDynamicData] todayStr jsonData 结果:', String(jsonData ? '有数据' : '空'));
if (jsonData && jsonData.departments) {
dynamicReportData = buildDynamicReportData(jsonData);
CURRENT_REPORT_DATE = todayStr;
logger.info('[initDynamicData] todayStr 构建成功, keys:', String(Object.keys(dynamicReportData)));
}
} catch (e) {
logger.warn('[initDynamicData] todayStr 加载失败，进入 fallback:', e.message);
}
...
logger.info('[initDynamicData] index.json 可用日期数:', { arg0: availableDates.length, arg1: '最新:', arg2: availableDates[0] });
}
} catch(e2) {
logger.warn('[initDynamicData] index.json 请求失败，走日期回溯:', String(e2));
}
...
found = true;
logger.info('[initDynamicData] ✅ 从 available_dates 加载成功:', String(tryDate));
break;
}
} catch(e3) {
logger.info(`[initDynamicData] available_dates[${i}] ${tryDate} 失败:`, e3.message);
}
}
}
if (!found) {
logger.info('[initDynamicData] 进入日期回溯（最多7天）...');
...
found = true;
logger.info('[initDynamicData] ✅ 回溯加载成功:', String(candidate));
break;
}
} catch(e3) {
logger.info(`[initDynamicData] 回溯 ${candidate} 失败:`, e3.message);
}
}
}
if (!found) {
logger.warn('[initDynamicData] 全部失败，使用硬编码 fallback');
dynamicReportData = reportData;
CURRENT_REPORT_DATE = '2026-04-28';
}
}
}
logger.info('[initDynamicData] 数据加载完成');
}
...
fetch(`reports/lfp_all_data.json?t=${Date.now()}`),
]);
...
logger.info('[碳酸锂期货]', { arg0: lcm?.name, arg1: lcm?.price || lcm?.prev_close, arg2: '元/吨' });
} catch (e) {
logger.warn('[碳酸锂期货] 加载失败:', e.message);
}
}
...
logger.info('[钢铁网数据] 碳酸锂:', { arg0: prices['碳酸锂']?.price, arg1: '| 磷酸铁:', arg2: prices['磷酸铁']?.price });
} catch (e) {
logger.warn('[钢铁网数据] 加载失败:', e.message);
}
}
}
...
logger.info('[初始化] 开始加载锂电Dashboard...');
try {
logger.info('[初始化] 加载产业链配置...');
...
logger.info('[初始化] ✅ 产业链配置加载成功');
logger.info('[初始化] 加载市场数据...');
...
logger.info('[初始化] ✅ 我的钢铁网数据加载成功');
} else {
logger.info('[初始化] ⚠️ 我的钢铁网数据加载失败');
}
if (lcResp.status === 'fulfilled' && lcResp.value.ok) {
const lcData = await lcResp.value.json();
const lcm = lcData.contracts?.find(c => c.name.includes('碳酸锂')) || lcData.contracts?.[0];
...
logger.info('[初始化] ✅ 碳酸锂期货数据加载成功');
}
} else {
logger.info('[初始化] ⚠️ 碳酸锂期货数据加载失败');
}
try {
...
logger.info('[初始化] ✅ 累积涨跌幅数据计算完成，共' + cumulativeData.products.length + '个品种');
} catch (e) {
logger.info('[初始化] ⚠️ 累积涨跌幅数据计算异常:', e.message);
}
...
logger.info('[初始化] ⚠️ 图表历史数据解析异常: ' + item.key, e.message);
}
}
}
logger.info('[初始化] ✅ chainHistoryData构建完成: ' + JSON.stringify(Object.keys(chainHistoryData)));
} catch (e) {
logger.info('[初始化] ⚠️ chainHistoryData构建异常:', e.message);
}
...
} catch (e) { logger.info('[初始化] ⚠️ 铁矿石数据解析异常:', e.message); }
}
if (lepidoliteResp && lepidoliteResp.status === 'fulfilled' && lepidoliteResp.value.ok) {
try {
...
} catch (e) { logger.info('[初始化] ⚠️ 锂云母数据解析异常:', e.message); }
}
if (oreResp && oreResp.status === 'fulfilled' && oreResp.value.ok) {
try {
...
logger.info('[初始化] ✅ 锂辉石精矿数据加载成功: price=' + latestPrice + ', mom=' + mom + '%');
}
}
} catch (e) { logger.info('[初始化] ⚠️ 锂辉石精矿数据解析异常:', e.message); }
}
...
logger.info('[初始化] ✅ 磷矿石30%品位数据加载成功: price=' + (latestPrice * 10000) + ', mom=' + mom + '%');
}
}
...
logger.info('[初始化] ✅ 硫酸亚铁数据加载成功: price=' + latestPrice + ', mom=' + mom + '%');
}
}
} catch (e) { logger.info('[初始化] ⚠️ 磷矿石/硫酸亚铁数据解析异常:', e.message); }
}
...
logger.info('[初始化] ✅ 新能源车数据加载成功: price=' + latestVal + ', mom=' + mom + '%');
}
}
} catch (e) { logger.info('[初始化] ⚠️ 新能源车数据解析异常:', e.message); }
}
...
logger.info('[初始化] ✅ 锂电池数据加载成功: price=' + (latestVal * 10000) + ', mom=' + mom + '%');
}
}
} catch (e) { logger.info('[初始化] ⚠️ 锂电池数据解析异常:', e.message); }
}
...
} catch(e) { logger.info('[初始化] ⚠️ 磷矿石数据解析异常:', e.message); }
}
cumulativeDataMetals.meta.generated_at = new Date().toISOString();
logger.info('[初始化] ✅ 有色金属累积涨跌幅数据计算完成，共' + cumulativeDataMetals.products.length + '个品种');
} catch (e) {
logger.info('[初始化] ⚠️ 有色金属累积涨跌幅数据计算异常:', e.message);
}
logger.info('[初始化] 最终chainHistoryData: ' + JSON.stringify(Object.keys(chainHistoryData)));
logger.info('[初始化] 市场数据: ' + JSON.stringify(Object.keys(marketData)));
...
if (loadingEl) {
loadingEl.remove();
logger.info('[初始化] ✅ 已移除 loading 提示');
}
} catch (error) {
logger.info('[初始化] ❌ 错误: ' + error.message);
logger.error('[初始化] 错误:', String(error));
const marketDashboard = document.querySelector('.market-dashboard');
if (marketDashboard) {
marketDashboard.classList.add('loaded');
logger.info('[初始化] ⚠️ 市场行情监控区域已强制显示（错误处理）');
}
document.querySelectorAll('#panel-lithium .mk-chart-grid').forEach(grid => {
grid.style.display = 'grid';
});
logger.info('[初始化] ⚠️ 已显示折线图作为后备');
}
}
function updatePageData(chainConfig, marketData) {
logger.info('[更新数据] 开始更新页面数据...');
...
logger.info('[更新数据] ✅ 页面数据更新完成');
}
function addChainPositionTags(chainConfig, marketData) {
logger.info('[位置标签] 开始添加产业链位置标签...');
...
logger.info('[位置标签] 检查卡片: ' + cardTitle);
let nodeName = null;
...
logger.info('[位置标签] ✅ 已添加标签到卡片: ' + cardTitle);
});
}
...
logger.info('[甘特图] 开始添加累积涨跌幅甘特图...');
if (!cumulativeData || !cumulativeData.products || cumulativeData.products.length === 0) {
logger.info('[甘特图] ⚠️ 无累积数据');
return;
}
const ganttContainer = document.getElementById('gantt-chart-container');
if (!ganttContainer) {
logger.info('[甘特图] ⚠️ 未找到gantt-chart-container');
return;
}
...
logger.info('[甘特图] 收起行 productName=', String(productName));
return;
}
...
logger.info('[generateGanttButtons] 开始生成按钮组...');
const btnGroup = document.getElementById('ganttBtnGroup');
if (!btnGroup) {
logger.error('[generateGanttButtons] 未找到 ganttBtnGroup');
return;
}
btnGroup.innerHTML = '';
...
logger.info('[generateGanttButtons] ✅ 按钮组已生成，共', { arg0: sortedProducts.length, arg1: '个按钮' });
}
...
logger.info('[selectGanttProduct] 选中品种：', String(productName));
...
logger.info('[showProductChart] productName=', String(productName));
const area = document.getElementById('gantt-expand-area');
if (!area) { logger.error('[showProductChart] 未找到 gantt-expand-area'); return; }
...
logger.error('[showProductChart] 未找到匹配 svgId, productName=', String(productName));
return;
}
...
logger.error('[showProductChart] 未找到SVG元素, svgId=', String(targetSvgId));
return;
}
...
logger.error('[showProductChart] 未找到卡片元素, svgId=', String(targetSvgId));
return;
}
...
if (!svgEl) { logger.warn('[折线动画] 未找到SVG'); return; }
const deepLines = Array.from(svgEl.querySelectorAll('.deep-line-anim'));
logger.info('[折线动画] 找到深色动画线数量:', { arg0: deepLines.length, arg1: '，设置CSS变量--line-len/--line-i' });
...
} catch(e) { logger.warn('[折线动画] 设置变量失败:', String(e)); }
});
}, 200);
...
logger.info('[showProductChart] ✅ 已移动卡片到展开区域, svgId=', String(targetSvgId));
const renderMap = {
...
try {
window[renderFnName]();
logger.info('[showProductChart] ✅ 已触发重新渲染:', String(renderFnName));
} catch(e) {
logger.warn('[showProductChart] ⚠️ 重新渲染失败:', { renderFnName: renderFnName, e: e });
}
}, 250);
} else {
logger.info('[showProductChart] ℹ️ 未找到渲染函数 for svgId=', { targetSvgId: targetSvgId, arg1: ', renderFnName=', renderFnName: renderFnName });
}
setTimeout(() => {
...
logger.info('[产业链图] 开始添加产业链全景图...');
const chainFlowEl = document.getElementById('chain-flow');
if (!chainFlowEl) {
logger.info('[产业链图] ⚠️ 未找到chain-flow元素');
return;
}
const nodeDbMap = {
...
logger.info('[findNodeData] ' + nodeName + ' → 别名命中 chainHistoryData[' + alias + ']');
...
logger.info('[findNodeData] ' + nodeName + ' → 模糊命中 chainHistoryData[' + ck + '] via alias ' + alias);
...
logger.info('[findNodeData] ' + nodeName + ' → 完全未命中，返回null');
return null;
}
...
logger.info('[产业链图] ✅ 已添加产业链全景图（分组层级布局）');
} else {
const nodes = chainConfig.nodes || [];
flowHTML += '<div class="chain-row">';
...
flowHTML += '</div>';
logger.info('[产业链图] ⚠️ 使用旧线性布局（无display配置）');
}
flowHTML += '</div>';
...
logger.info('显示市场详情:', String(type));
}
function getReportData(deptId) {
if (dynamicReportData && dynamicReportData[deptId]) {
logger.info('[getReportData] ✓ 命中 dynamicReportData[' + deptId + '], content长度=' + dynamicReportData[deptId].content.length);
return dynamicReportData[deptId];
}
logger.warn('[getReportData] ✗ dynamicReportData无数据，fallback: reportData[' + deptId + ']=' + (!!reportData[deptId]));
...
logger.info('[openReport] deptId=' + deptId + ', contentHtml前1000字符=', String(contentHtml.substring(0, 1000)));
logger.info('[openReport] deptId=' + deptId + ', contentHtml后500字符=', String(contentHtml.substring(contentHtml.length - 500)));
if (autoPlay) {
...
logger.info('closeReport() called');
try {
stopAutoPlay();
document.getElementById('reportModal').classList.remove('active');
document.body.classList.remove('report-modal-open');
document.body.style.overflow = 'auto';
} catch(e) { logger.error('Error closing report:', String(e)); }
}
...
const grid = document.getElementById('allbuGrid');
if (!grid) return;
logger.info('[_doRenderAllBu] dynamicReportData keys:', String(dynamicReportData ? Object.keys(dynamicReportData) : 'NULL'));
...
if (!d) {
logger.info('[_doRenderAllBu] Missing data for:', String(id));
...
toast('链接已复制到剪贴板，可以分享给他人了！');
}
}
let calendarYear = 2026, calendarMonth = 3;
...
logger.error('加载历史数据失败:', String(error));
...
.catch(error => {
logger.error('加载早报失败:', String(error));
toast('加载早报失败，请稍后重试');
});
}
...
.catch(error => {
logger.error('加载分类视图失败:', String(error));
toast('加载分类视图失败');
});
}
...
if (!dynamicReportData) {
logger.error('[ERROR] dynamicReportData 为空');
}
} catch(e) {
logger.error('[ERROR] initDynamicData 异常:', String(e));
}
const loadingOverlay = document.getElementById('loadingOverlay');
if (loadingOverlay) {
loadingOverlay.classList.add('hidden');
...
logger.warn('[LC Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
logger.warn('[Battery Grade Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
logger.warn('[Industrial Grade Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
logger.warn('[LC Chart] end marker error:', String(e));
}
var selG = svg.querySelector('#' + prefix + 'ChartSelection');
if (!selG) {
selG = document.createElementNS('http:
...
if (!chG) {
chG = document.createElementNS('http:
...
logger.warn('[LFP Dual Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
logger.warn('[LFP Dual Chart] end marker error:', String(e));
}
...
if (!crosshairG) {
crosshairG = document.createElementNS('http:
...
logger.warn('[FP Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
logger.warn('[WTI Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
logger.error('[renderWTIChart] draw error:', String(e));
}
...
logger.error('[renderUreaChart] draw error:', String(e));
}
var pw = (svg.parentElement && svg.parentElement.clientWidth) || 800;
svg.setAttribute('width', pw);
svg.setAttribute('height', Math.round(pw * 260 / 800));
...
logger.warn('[UR Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
logger.warn('[PT Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
triggerLineRevealAnimation(svg);
} catch(e) { logger.error('[renderPTChart] draw error:', String(e)); }
var pw = (svg.parentElement && svg.parentElement.clientWidth) || 800;
svg.setAttribute('width', pw);
svg.setAttribute('height', Math.round(pw * 260 / 800));
}
...
logger.warn('[PD Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
triggerLineRevealAnimation(svg);
} catch(e) { logger.error('[renderPDChart] draw error:', String(e)); }
var pw = (svg.parentElement && svg.parentElement.clientWidth) || 800;
svg.setAttribute('width', pw);
svg.setAttribute('height', Math.round(pw * 260 / 800));
}
...
logger.warn('[I Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
triggerLineRevealAnimation(svg);
} catch(e) { logger.error('[renderIChart] draw error:', String(e)); }
var pw = (svg.parentElement && svg.parentElement.clientWidth) || 800;
svg.setAttribute('width', pw);
svg.setAttribute('height', Math.round(pw * 260 / 800));
}
...
logger.warn('[Phosphate Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
triggerLineRevealAnimation(svg);
} catch(e) { logger.error('[renderPhosphateChart] draw error:', String(e)); }
var pw = (svg.parentElement && svg.parentElement.clientWidth) || 800;
svg.setAttribute('width', pw);
svg.setAttribute('height', Math.round(pw * 260 / 800));
}
...
logger.warn('[EG Chart] load failed:', String(e));
if (loading) loading.style.display = 'none';
if (noData) noData.style.display = 'flex';
}
}
...
triggerLineRevealAnimation(svg);
} catch(e) { logger.error('[renderEGChart] draw error:', String(e)); }
var pw = (svg.parentElement && svg.parentElement.clientWidth) || 800;
svg.setAttribute('width', pw);
svg.setAttribute('height', Math.round(pw * 260 / 800));
}
...
logger.warn('[FP Chart] end marker error:', String(e));
}
var step = Math.ceil(dataLength / 6);
...
if (!hist) return [];
return Object.keys(hist).sort().reverse();
}
...
lubricant: [0.18, 0.22, 0.15, 0.22, 0.15, 0.08],
sjld:   [0.15, 0.18, 0.15, 0.17, 0.25, 0.10],
sjl:    [0.15, 0.18, 0.15, 0.17, 0.25, 0.10],
dhx:    [0.20, 0.22, 0.18, 0.15, 0.15, 0.10],
dkhx:   [0.20, 0.22, 0.18, 0.15, 0.15, 0.10],
lpsd:   [0.20, 0.20, 0.18, 0.17, 0.13, 0.12],
kls:    [0.18, 0.22, 0.15, 0.22, 0.15, 0.08],
kelan:  [0.18, 0.22, 0.15, 0.22, 0.15, 0.08],
czly:   [0.20, 0.20, 0.18, 0.17, 0.13, 0.12],
bych:   [0.20, 0.20, 0.18, 0.17, 0.13, 0.12],
};
...
return Math.min(120, (item.curr / item.prev) * 100);
} else {
if (isCost) {
if (!item.actual || item.actual <= 0) return 0;
return Math.min(120, (item.target / item.actual) * 100);
} else {
if (!item.target || item.target <= 0) return 0;
return Math.min(120, (item.actual / item.target) * 100);
}
}
}
...
kpis.d1 = [
achvToScore(avgAchv),
achvToScore(profitAchv),
achvToScore(costAchv),
achvToScore(revAchv)
];
} else {
kpis.d1 = [80, 80, 80, 80];
...
if (dept) {
logger.info('[getLatestReportForBU] 使用嵌入数据, buId=' + buId);
return { ...dept, _reportDate: window.__EMBEDDED__.today };
}
}
logger.info('[getLatestReportForBU] 开始加载, buId=' + buId);
let availableDates = [];
try {
...
logger.info('[getLatestReportForBU] 无法加载 index.json, 使用最近3天');
}
...
if (dept) {
logger.info('[getLatestReportForBU] 找到报告, buId=' + buId + ', date=' + date);
return {...dept, _reportDate: date};
}
} else {
logger.info('[getLatestReportForBU] 跳过（无报告）: ' + date + ', status=' + res.status);
}
} catch (e) {
logger.info('[getLatestReportForBU] 跳过（加载失败）: ' + date + ', error=' + e.message);
}
}
logger.info('[getLatestReportForBU] 无可用数据, buId=' + buId);
logger.info('[getLatestReportForBU] 无可用数据, buId=' + buId);
...
<svg class="radar-svg" viewBox="0 0 160 160" xmlns="http:
<defs>
...
function closePanel() {
logger.info('closePanel() called');
try {
document.getElementById('panelOverlay').classList.remove('active');
} catch(e) { logger.error('Error closing panel:', String(e)); }
try {
...
logger.info('[锂电子分类] 已切换到: ' + subCategory + ', 产业链全景图: ' + (subCategory === 'lfp' ? '显示' : '隐藏'));
}
...
logger.info('[甘特图] 渲染电解液甘特图...');
logger.info('[甘特图] ELECTROLYTE_DATA定义:', String(typeof ELECTROLYTE_DATA !== 'undefined'));
logger.info('[甘特图] ELECTROLYTE_VARIETIES定义:', String(typeof ELECTROLYTE_VARIETIES !== 'undefined'));
if (typeof ELECTROLYTE_DATA === 'undefined') {
logger.info('[甘特图] ⚠️ ELECTROLYTE_DATA未定义');
container.innerHTML = '<div style="padding:40px;text-align:center;color:#e74c3c;">⚠️ 电解液数据未加载（请检查electrolyte_data.js）</div>';
return;
}
logger.info('[甘特图] ELECTROLYTE_DATA keys:', String(Object.keys(ELECTROLYTE_DATA)));
logger.info('[甘特图] ELECTROLYTE_VARIETIES:', String(ELECTROLYTE_VARIETIES));
...
logger.info('[电解液甘特图] ✅ 已添加累积涨跌幅甘特图');
generateElectrolyteGanttButtons();
logger.info('[电解液甘特图] 准备绑定点击事件，共', { arg0: container.querySelectorAll('.gantt-row').length, arg1: '行' });
container.querySelectorAll('.gantt-row').forEach(row => {
row.style.cursor = 'pointer';
row.addEventListener('click', function() {
const productName = this.getAttribute('data-product-name');
logger.info('[电解液甘特图] 点击行 productName=', String(productName));
if (this.classList.contains('active')) {
this.classList.remove('active');
...
logger.info('[generateElectrolyteGanttButtons] 开始生成按钮组...');
const btnGroup = document.getElementById('electrolyteGanttBtnGroup');
if (!btnGroup) {
logger.error('[generateElectrolyteGanttButtons] 未找到 electrolyteGanttBtnGroup');
return;
}
btnGroup.innerHTML = '';
...
logger.info('[generateElectrolyteGanttButtons] ✅ 按钮组已生成，共', { arg0: sortedProducts.length, arg1: '个按钮' });
}
...
logger.info('[selectElectrolyteGanttProduct] 选中品种：', String(productName));
...
logger.info('[showElectrolyteProductChart] productName=', String(productName));
const area = document.getElementById('electrolyte-gantt-expand-area');
if (!area) { logger.error('[showElectrolyteProductChart] 未找到 electrolyte-gantt-expand-area'); return; }
area.style.display = 'block';
...
ctx.fill();
const dateText = lastPoint.date.slice(5);
...
logger.info('[renderElectrolyteLineChart] ===== START =====');
logger.info('[renderElectrolyteLineChart] svgId=', { svgId: svgId, arg1: 'data长度=', arg2: data ? data.length : 0 });
logger.info('[renderElectrolyteLineChart] svg=', { arg0: !!svg, arg1: 'svg.id=', arg2: svg ? svg.id : 'N/A' });
if (!data || data.length === 0) {
logger.error('[renderElectrolyteLineChart] 数据为空!');
return;
}
logger.info('[renderElectrolyteLineChart] 第1条:', JSON.stringify(data[0]));
logger.info('[renderElectrolyteLineChart] 最后1条:', JSON.stringify(data[data.length - 1]));
const W = 800, H = 260;
...
logger.info('[renderElectrolyteLineChart] gridG=', { arg0: !!gridG, arg1: 'areaG=', arg2: !!areaG, arg3: 'lineG=', arg4: !!lineG, arg5: 'axisXG=', arg6: !!axisXG, arg7: 'axisYG=', arg8: !!axisYG });
if (gridG) gridG.innerHTML = '';
if (areaG) areaG.innerHTML = '';
if (lineG) lineG.innerHTML = '';
if (axisXG) axisXG.innerHTML = '';
if (axisYG) axisYG.innerHTML = '';
if (!lineG) {
logger.error('[renderElectrolyteLineChart] lineG不存在，创建新的!');
lineG = document.createElementNS('http:
lineG.setAttribute('id', svgId + 'Line');
lineG.setAttribute('class', 'deep-line-anim');
svg.appendChild(lineG);
}
function mk(tag, attrs) {
const el = document.createElementNS('http:
...
logger.info('[renderElectrolyteLineChart] 渲染完成，lineG子元素数量:', String(lineG ? lineG.children.length : 0));
}
