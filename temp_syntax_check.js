        ...
        function drawLithiumOreChart(svg, historyData) {
            ...
            lineG.appendChild(mk('path', { d: linePath, fill: 'none', stroke: '#ef5350', 'stroke-width': 2 }));
            ...
            if (container) {
                ...
                var hoverTip = document.getElementById('lithiumOreChartHoverTip');
                if (!hoverTip) {
                    hoverTip = document.createElement('div');
                    hoverTip.id = 'lithiumOreChartHoverTip';
                    hoverTip.className = 'chart-tool-tip';
                ...
                svg.addEventListener('mousemove', function(e) {
                    ...
                    hoverTip.innerHTML =
                        ...
                        '<span style="font-size:13px;font-weight:bold">' + d.avg_price.toFixed(0) + ' ' + (d.unit || '美元/吨') + '</span><br>' +
                        '<span style="color:' + chgColor + ';font-size:11px">' + chgSign + chgPct.toFixed(2) + '%</span>';
                    ...
                    hoverTip.style.top = (yScale(d.avg_price) / H * 100) + '%';
                });
        ...
        async function initLepidoliteChart() {
            ...
            try {
                ...
                if (!data.historyy || data.history.length === 0) {
                    if (loading) loading.style.display = 'none';
                    if (noData) noData.style.display = 'flex';
                    return;
                }
        ...
        function drawLepidoliteChart(svg, historyData) {
            ...
            if (container) {
                container.style.position = 'relative';
                var hoverTip = document.getElementById('lepidoliteChartHoverTip');
                if (!hoverTip) {
                    hoverTip = document.createElement('div');
                ...
                svg.addEventListener('mousemove', function(e) {
                    ...
                    hoverTip.innerHTML =
                        '<span style="color:#9bb5d4;font-size:11px">' + d.date + '</span><br>' +
                        '<span style="font-size:13px;font-weight:bold">' + d.avg_price.toFixed(0) + ' ' + (d.unit || '元/吨') + '</span><br>' +
                        '<span style="color:' + chgColor + ';font-size:11px">' + chgSign + chgPct.toFixed(2) + '%</span>';
                    hoverTip.style.display = 'block';
                    var xp = xScale(idx) / W;
                    if (xp > 0.65) {
